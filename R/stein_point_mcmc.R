...
#' the whole state space with a short Markov chain. At step `j`, the algorithm
#' first chooses one of the already selected points as the start of the chain.
#' The chain is run for `m_seq[j]` states, duplicate states are removed, and
#' each remaining candidate `x` is scored by the greedy Stein Points objective.
#' The target score is written \eqn{s_p(x)=\nabla_x\log p(x)}, and the Stein
#' kernel `k0` is the same object described in [stein_kernel_matrix()]. For a
...
#'
#' The first point is fixed by `x_init`, so `n_points - 1` short chains are used
#' to add the remaining points. A scalar `m_seq` is recycled as the chain length
#' for every update; a vector of length `n_points - 1` supplies the update
...
#' cached matrix `K0` with entries `K0[a,b]=k0(x_a,x_b)` for already selected
#' points; this is why start rules, candidate scoring, and KSD diagnostics can
#' reuse the same pairwise Stein information.
#'
#' The built-in start rules are `"last"` for the most recently selected point,
#' `"rand"` for a random selected point, and `"infl"` for the point with the
...
#' two choices used in the SP-MCMC paper. Custom `criterion`, `transition_fn`,
#' and `proposal_fn` arguments expose the same pieces separately so users can
...
#' `counts`, report cumulative evaluations in `cum_n_eval`, and record
...
#' the main loop. [sp_mcmc_state()] builds the state passed to a start rule,
#' [sp_mcmc_criterion()] turns LAST/RAND/INFL or a custom function into a
#' reusable selector, [sp_mcmc_select_start()] validates the selected row index,
#' and [sp_mcmc_eval_candidates()] scores the candidate path using the Stein
#' Points objective. Moving sideways from [stein_points()] to `sp_mcmc()` keeps
#' the same greedy objective but replaces the optimizer with an MCMC transition
#' such as [grw()], [rwm()], or [mala()].
#'
...
#' @param m_seq Chain length at each selection step. Supply one value, one value
...
#' @param Sigma Proposal covariance or preconditioning matrix. If `NULL`, the
...
#' @param Sigma Proposal covariance or preconditioning matrix.
