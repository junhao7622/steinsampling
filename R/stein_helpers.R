...
# Sample row indices without changing the caller's random state.
...
#'
#' Computes the median squared Euclidean distance between sample rows. This is
#' the median-heuristic squared scale used by the built-in kernels when the user
#' does not supply a scale.
#'
#' @details
#' If the sample rows are `z_1, ..., z_n` in `R^d`, the full calculation forms
#' the vector of all off-diagonal squared Euclidean distances
#' \deqn{
#'   D = \{ ||z_i - z_j||_2^2 : 1 \le i < j \le n \}.
#' }
#' The return value is `median(D)`, not its square root. Thus for one-dimensional
#' input `c(-1, 0, 2)`, the pairwise squared distances are `1`, `9`, and `4`,
#' and the returned value is `4`. For matrix input, rows are observations and
#' columns are coordinates; a numeric vector is treated as an `n x 1` sample.
#'
...
#' For the Gaussian RBF kernel this means `h^2 = median(D)` in
#' `exp(-||x - y||^2 / (2 h^2))`. For the IMQ kernel it means
#' `c^2 = median(D)` in `(c^2 + ||x - y||^2)^beta`. The function does not
...
#'
#' If `use_sampling = TRUE` and `nrow(Z) > max_samples`, only `max_samples` rows
#' are sampled without replacement before the distance vector is formed. This
#' changes the computational cost from all \eqn{n(n - 1) / 2} pairs to all
#' \eqn{m(m - 1) / 2} pairs for `m = max_samples`. The `seed` argument is used
#' only in that subsampling case. If all rows are used, the result is
#' deterministic and `seed` has no effect.
#'
...
#' @param max_samples Maximum number of rows used when subsampling is enabled.
#' @param use_sampling If `TRUE`, use at most `max_samples` rows.
#' @param seed Optional seed used when rows are sampled.
#'
#' @return
#' A positive numeric scalar. It is the median of the finite off-diagonal
#' squared Euclidean distances after any optional subsampling. The value is on
#' the squared-distance scale: use `sqrt(find_median_distance(Z))` only if a
#' distance-scale bandwidth is needed. If all finite pairwise squared distances
#' are zero, the returned value is `1e-5`; if no finite distance can be formed,
#' the returned fallback is `1`.
#' @examples
#' find_median_distance(c(-1, 0, 2), use_sampling = FALSE)
#'
#' Z <- matrix(c(0, 0, 1, 0, 0, 2), ncol = 2, byrow = TRUE)
#' find_median_distance(Z, use_sampling = FALSE)
#'
#' set.seed(1)
#' big_Z <- matrix(rnorm(300), ncol = 3)
#' find_median_distance(big_Z, max_samples = 20, seed = 10)
#' @export
