...
#'
#' @details
#' The paper defines FSSD by choosing `J` locations \eqn{v_1,\ldots,v_J} and checking
#' whether the Stein witness function is zero at those locations. This manual
#' writes the target density as `p` and its score as
#' \eqn{s_p(x) = \nabla_x \log p(x)}. For each sample point `x_i` and location
#' `v_j`, the package builds the Stein feature
#' \deqn{\xi_p(x_i, v_j) = s_p(x_i) k(x_i, v_j) + \nabla_x k(x_i, v_j).}
#' Here `k` is the selected base kernel and the gradient is with respect to the
#' sample argument `x_i`, not the location argument. For `d`-dimensional data,
#' each location contributes a length-`d` vector.
#'
#' Let \eqn{\Xi(x)} be the `d x J` matrix whose column `j` is
#' \eqn{\xi_p(x, v_j) / \sqrt{dJ}}. The vector
#' \eqn{\tau(x) = vec(\Xi(x))} stacks the columns of \eqn{\Xi(x)} into length
#' `d * J`; this is the row representation returned by [compute_tau()]. For
#' two sample rows,
#' \deqn{\Delta(x, y) = \tau(x)^T \tau(y)
#'      = \frac{1}{dJ}\sum_{j=1}^J
#'        \xi_p(x, v_j)^T \xi_p(y, v_j)}
#' is the FSSD U-statistic kernel. The division by \eqn{\sqrt{dJ}} at the
#' feature level is why the inner product includes the \eqn{1/(dJ)} factor used
#' in the FSSD paper.
#'
#' The test statistic is an average inner product of these features over pairs
#' of different sample points:
#' \deqn{
#' \widehat{FSSD}^2 =
#' \frac{1}{n(n - 1)} \sum_{i \ne l} \tau(x_i)^T \tau(x_l).
#' }
#' In matrix form, [compute_tau()] returns the `n x (d * J)` matrix whose
#' `i`th row is `tau(x_i)`. The unbiased statistic above is computed from this
#' matrix without explicitly storing all pairwise inner products.
#'
#' The reported p-value uses the asymptotic null distribution from the paper.
#' The covariance of the feature matrix is estimated from the data as
#' \eqn{\hat\Sigma = n^{-1}\sum_i(\tau(x_i)-\bar\tau)
#' (\tau(x_i)-\bar\tau)^T}. Its eigenvalues are computed, and null draws are
#' simulated as \eqn{\sum_j \lambda_j(Z_j^2 - 1)}. Large values mean the sample
#' and target score do not agree well. The `"opt"` variant uses part of the data
#' to choose useful locations before computing the final statistic.
#'
#' This test is parallel to the KSD tests but uses a different compression of
#' the Stein information. [ksd_u_test()] and [ksd_v_test()] build all pairwise
#' values `k0(x_i, x_j)` with [stein_kernel_matrix()]. FSSD instead chooses a
#' finite location matrix `V` and builds the feature matrix with [compute_tau()].
#' Moving from KSD to FSSD keeps the same sample `X`, target score convention,
#' and kernel layer, but adds the number of test locations `J` and, for
#' `"opt"`, a training stage that chooses `V` and the built-in kernel scale.
#'
...
#' @param scaling Optional positive squared kernel scale. `NULL` uses the median
#'   squared distance between sample pairs.
#' @param train_ratio Fraction of samples used to tune FSSD-opt.
...
#' @param median_max_samples,median_use_sampling,median_seed Options passed to
#'   [find_median_distance()] when a default scale is needed.
...
#'
#' The `htest` fields make the object print like a hypothesis test. The extra
#' `info` field is included because FSSD depends on chosen test locations; a
#' p-value without the locations `V` is hard to reproduce or interpret.
#' @examples
...
#' locations are drawn from a Gaussian fitted to the training rows. Built-in
#' kernels also try a short grid of squared scales before bounded L-BFGS-B
#' refinement of the locations, and of the squared kernel scale when applicable.
...
#' labeled `fssd`), `p.value`, `method`, `data.name`, and `parameter`. The
#' `info` field is a
#' list with `variant = "opt"`, the optimized location matrix `V`, the kernel
#' name, optimized squared scale `sigma2_opt` when applicable, optimized
#' objective value, iteration count, and L-BFGS-B convergence code. The statistic
#' is computed only on the held-out test rows.
#'
...
#' * `p_value`: fraction of simulated null draws larger than `test_score`.
...
#' training data. For built-in kernels it then tries a short grid of squared
#' scales and uses the best grid value as the starting scale. The final step is
#' bounded optimization of the packed location coordinates and the square root
#' of the squared scale.
#'
...
#' @param median_max_samples,median_use_sampling,median_seed Options used when
#'   building the initial scale grid.
#'
...
#'
#' @details
#' If `scaling` is missing, the median squared distance between sample rows is
#' used. For Gaussian RBF kernels this sets `h^2`; for IMQ kernels this sets
#' `c^2`. When a kernel object is supplied, assigning a missing scale preserves
#' its other settings, including its preconditioner.
#'
...
#' @param imq_beta IMQ exponent.
#'
#' @return A list with `kernel_obj`, `kernel_name`, and `scaling`.
#' @noRd
...
#' Build a default grid of FSSD kernel scales
#'
#' Creates five squared scales around the median squared distance:
#' `median * 2^seq(-3, 3, length.out = 5)`.
#'
#' @param X Numeric sample matrix.
#' @param max_samples Maximum number of rows used for the median calculation.
#' @param use_sampling Logical; whether to subsample rows before computing the
#'   median distance.
#' @param seed Optional seed for row subsampling.
#'
...
#'
#' @param train_ratio Fraction of rows used for FSSD-opt training.
#'
