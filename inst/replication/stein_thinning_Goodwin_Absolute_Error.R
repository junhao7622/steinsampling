...
has_support <- requireNamespace("support", quietly = TRUE)
if (!has_support) {
  warning(
...
quiet_sp <- function(...) {
  if (!has_support) {
    stop("Package 'support' is unavailable.", call. = FALSE)
  }
...
support_means <- if (has_support) {
  ...
  matrix(NA_real_, nrow = length(m_grid), ncol = ncol(X))
}
...
if (!has_support) {
  legend_labels["support"] <- "Support Points (unavailable)"
}
